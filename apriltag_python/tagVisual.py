...
class Canvas(app.Canvas):
    def __init__(self, **kwargs):
        ...
        app.Canvas.__init__(self, keys="interactive", size=(960, 960), **kwargs)
        ps = self.pixel_scale
        #######################################
        n = 8
        data = np.zeros(
    ...
    def on_key_press(self, event):
        if event.text == "w":
            self.phi += 2
            self.model = np.dot(
                rotate(self.theta, (0, 0, 1)), rotate(self.phi, (0, 1, 0))
            )
        if event.text == "s":
            self.phi -= 2
            self.model = np.dot(
    ...
    def detector_im(self):
        ...
        if (
            len(detections) < 1
            or len(detections1) < 1
            or len(detections2) < 1
