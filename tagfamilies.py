...
class tagclass(object):
    ...
    def __interpolate(self,p, relativePoint):
        """
    ...
    def __decode(self, tagcode, points):
        """
        ...
        for r in range(4):
            ...
            for tag in self.tagcode:
                ...
                if (dis < besthamming):
                    ...
                    bestrotation = r
                index += 1
            rcodes = self.__rotate90(rcodes, self.d)
        ...
        if besthamming <= self.hammingdis:
    ...
    def decodeQuad(self, quads, gray):
        ...
        whitepoint = []
        for quad in quads:
            dd = 2 * self.blackBorder + self.d  # tagFamily.d
            blackvalue = []
            whitevalue = []
            tagcode = 0
            for iy in range(dd):
                for ix in range(dd):
                    x = (ix + 0.5) / dd
                    y = (iy + 0.5) / dd
                    point = np.int32(self.__interpolate(quad, (x, y)))
                    points.append(point)
            ...
            for iy in range(dd):
                for ix in range(dd):
                    ...
                    if value > threshold:
                        ...
                        tagcode |= 1
            tagcode = hex(tagcode)
            detection = self.__decode(tagcode, quad)
